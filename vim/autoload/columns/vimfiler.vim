...
func! vimfiler#columns#gitstatus#define() abort
	return s:column
endfunc"}}}
...
func! s:column.length(files, context) abort
	return 3
endfunc
...
" @vimlint(EVL103, 1, a:context)
func! s:column.define_syntax(context) abort
	...
	hi def link  vimfilerColumn__GitModified Special
	hi def link  vimfilerColumn__GitStaged   Function
	hi def link  vimfilerColumn__GitUnstaged Text
	hi def link  vimfilerColumn__GitRenamed  Title
	hi def link  vimfilerColumn__GitUnmerged Label
	hi def link  vimfilerColumn__GitDeleted  Text
	hi def link  vimfilerColumn__GitDirty    Tag
	hi def link  vimfilerColumn__GitClean    DiffAdd   
	hi def link  vimfilerColumn__GitUnknown  Text   
endfunc
" @vimlint(EVL103, 0, a:context)

func! s:directory_of_file(file) abort
...
func! s:git_shellescape(arg) abort
	if a:arg =~# '^[A-Za-z0-9_/.-]\+$'
		return a:arg
...
func! s:cmd_in_directory_of_file(file, cmd) abort
	return 'cd '.s:git_shellescape(s:directory_of_file(a:file)) . (s:fish ? '; and ' : ' && ') . a:cmd
endfunc



func! s:git_state_to_name(symb)  abort
	if a:symb ==# '?'
		return 'Untracked'
	elseif a:symb ==# ' '
		return 'Modified'
...
func! s:git_state_to_symbol(s) abort
	let name = s:git_state_to_name(a:s)
	return g:VimFilerGitIndicatorMap[name]
endfunc

" @vimlint(EVL103, 1, a:context)
func! s:column.get(file, context) abort
